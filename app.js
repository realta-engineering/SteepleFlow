...
function cycleRow(c) {
  ...
  return `<div class="cycle-row"><div><div class="cycle-title"><strong>${esc(c.name)}</strong><span class="status ${esc(c.status)}">${esc(c.status)}</span></div><div class="cycle-meta"><span>${icon("calendar-days")} ${dateLabel(c.start)} - ${dateLabel(c.end)}</span><span>${icon("users")} ${responseCount} ${responseCount === 1 ? "response" : "responses"}</span></div></div><button class="icon-btn" title="Open cycle" data-cycle="${c.id}">${icon("chevron-right")}</button></div>`;
}
...
function renderParticipants() {
  ...
  const submitted = people.filter(p=>p.submitted).length;
  document.querySelector("#view").innerHTML = `
    <div class="page-head"><div><h2>Participant availability</h2><p>${esc(cycle.name)} &middot; ${submitted} ${submitted === 1 ? "response" : "responses"}</p></div><div class="page-actions"><button class="btn btn-primary" data-copy-link>${icon("link")} Copy submission link</button></div></div>
...
function showCycleModal() {
  ...
  document.querySelector("#cycle-form").addEventListener("submit",async e=>{e.preventDefault();const fd=new FormData(e.target);const roles=[...document.querySelectorAll("[name='role']")].map(i=>i.value.trim()).filter(Boolean);const start=fd.get("start"),end=fd.get("end");const payload={churchId:church().id,name:fd.get("name"),start,end,roles,dates:sundaysBetween(start,end)};let c;try{const result=await api.call("createCycle",payload);if(!result.cycle)throw new Error("The server did not return the created cycle.");c=result.cycle}catch(error){toast(error.message,"circle-alert");return}store.state.activeCycleId=c.id;store.state.cycles.unshift(c);store.save();closeModal();location.hash="cycles";route();toast("Cycle created","calendar-check")}); refreshIcons();
}
